...
data "aws_ssm_parameter" "private_subnet_ids" {
  name = "${var.vpc_ssm_id}/private_subnet_ids"
}
data "aws_ssm_parameter" "vpc_id" {
  name = "${var.vpc_ssm_id}/vpc_id"
}
locals {
  enabled  = var.enabled # module.this.enabled
...
locals {
  id = "${var.orgname}-${var.region_name}-${var.environment}-${var.project_name}-${var.resourcename}-${var.resource_desc}"
}

locals {
  vpc_id     = nonsensitive(data.aws_ssm_parameter.vpc_id.value)
  subnet_ids = nonsensitive(split(",", data.aws_ssm_parameter.private_subnet_ids.value))
}
